    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-semibold text-slate-500">Buscando dados vacinais...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-lg font-bold tracking-tight text-slate-900">MeuPetDigital</span>
              <p className="text-[10px] font-medium text-slate-400">Saude Preventiva Canina</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-red-600"
              type="button"
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-col gap-4">
                {filteredPets.map((pet) => {
                  ...
                  return (
                    <button
                      key={pet.id}
                      className="group flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-4 text-left shadow-sm transition hover:border-slate-200 hover:shadow-md"
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-medium text-slate-400">
                            {pet.raca || 'Sem raca definida'} {pet.peso ? `- ${pet.peso}kg` : ''}
                          </p>
                          {pet.rg_sinpatinhas && (
                            <span className="mt-1 inline-flex items-center gap-1 rounded bg-slate-50 px-1 py-0.5 text-[9px] font-semibold text-slate-500">
                      ...
                      <StatusBadge bgClass={ui.bgClass} dotClass={ui.dotClass} label={ui.label} />
                    </button>
                  );
                })}
              </div>
                  ...
                  <table className="w-full border-collapse text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredRegistros.map((registro) => {
                        const ui = getStatusUI(registro.status);
                        return (
        ...
        <section className="mt-12 rounded-2xl bg-emerald-800 p-6 text-white shadow-xl">
...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <span className="mt-4 block text-3xl font-black text-slate-950">{value}</span>
    </div>
  );
}

function StatusBadge({ bgClass, dotClass, label }: { bgClass: string; dotClass: string; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold ${bgClass}`}>
